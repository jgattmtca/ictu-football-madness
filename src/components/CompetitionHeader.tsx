...
interface Props { competition: Competition & { commissioner_message?: string } }
      ...
      <header className="border-b border-white/10 bg-black/30 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/admin" className="text-xs text-white/30 hover:text-white/60 transition-colors">
              Admin ⚙️
            </Link>
          </div>
        </div>
      </header>
      {competition.commissioner_message && (
        <div className="bg-purple-900/40 border-b border-purple-500/30">
          <div className="max-w-5xl mx-auto px-4 py-2.5 flex items-center gap-2">
            <span className="text-lg">📣</span>
            <p className="text-purple-200 text-sm">{competition.commissioner_message}</p>
          </div>
        </div>
      )}
    </>
